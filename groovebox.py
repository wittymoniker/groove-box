...
class PhaseLockedWavefieldEngine:
    ...
    def compute_wavefield(self):
        ...
        for i, name in enumerate(names):
            ...
            phase_field, harmonic_stretch, harmonic_balance, shape_field = [], [], [], []
            for s in range(count):
                u = app._seed_geometry(f"wavefield|{name}|{i}|{s}") if hasattr(app, "_seed_geometry") else (0.5,0.5,0.5)
                q = 2.0*np.pi*(u[0]*PHI + u[1]*SQRT2 + u[2]*MEUM_LOG2 + (i+1)*PHI_INV)
                phase_field.append(float(np.mod(q, 2*np.pi)))
                harmonic_stretch.append(float(1.0 + 0.095*np.sin(q*MEUM + (s+1)*PHI_INV)))
                harmonic_balance.append(float(0.5 + 0.5*np.sin(q*SQRT2 + MEUM)))
                shape_field.append(float(0.5 + 0.5*np.cos(q*PHI + (s+1)*MEUM_NORM)))
            self.wavefield[name] = {
                "euclidean": euc,
    ...
    def get_hints(self, instrument_name, step_index):
        ...
        Returns deterministic Euclidean + waveform-geometry fields; no synthetic
        amplitude envelope is generated by the wavefield.
        """
        ...
        ph = wf.get("phase_field") or []
        hs = wf.get("harmonic_stretch") or []
        hb = wf.get("harmonic_balance") or []
        sh = wf.get("shape_field") or []
        return {
            "euclidean": bool(euc[s]) if s < len(euc) else False,
            "phase": float(ph[s]) if s < len(ph) else 0.0,
            "harmonic_stretch": float(hs[s]) if s < len(hs) else 1.0,
            "harmonic_balance": float(hb[s]) if s < len(hb) else 0.5,
            "shape": float(sh[s]) if s < len(sh) else 0.5,
        }
    ...
    def apply_phase_locked_randomization(self):
        ...
        for name, wf in self.wavefield.items():
            ...
            euc = wf['euclidean']
            phf = wf.get('phase_field', [])
            hsf = wf.get('harmonic_stretch', [])
            hbf = wf.get('harmonic_balance', [])
            shf = wf.get('shape_field', [])
            ...
            for s in range(count):
                ...
                phase_cell = float(phf[s]) if s < len(phf) else 0.0
                stretch = float(hsf[s]) if s < len(hsf) else 1.0
                balance = float(hbf[s]) if s < len(hbf) else 0.5
                shape = float(shf[s]) if s < len(shf) else 0.5
                seed_pitch = 2.0 ** (
                    0.24 * math.sin(phase_cell + (op_idx + 1) * PHI_INV)
                    + 0.10 * (balance - 0.5)
                    + 0.06 * (stretch - 1.0)
                )

                steps[s] = on
                # Phase-Lock does not manufacture a volume envelope. Keep existing
                # engine amplitude/probability where available; only ensure generated
                # active cells have a deterministic nonzero value.
                if on:
                    if s >= len(amps) or amps[s] <= 0:
                        amps[s] = 0.72
                    if s >= len(probs) or probs[s] <= 0:
                        probs[s] = 100
                else:
                    amps[s] = 0.0
                    probs[s] = 0
                pitches[s] = float(np.clip(seed_pitch, 0.56, 1.78))
                mem.setdefault('phase_cells', [0.0] * count)[s] = phase_cell
                mem.setdefault('harmonic_stretch', [1.0] * count)[s] = stretch
                mem.setdefault('harmonic_balance', [0.5] * count)[s] = balance
                mem.setdefault('wave_shape', [0.5] * count)[s] = shape
                corrected += 1
...
class MathematiciansGrooveboxApp(QMainWindow):
    def __init__(self):
        ...
        # Phase-Lock waveform geometry is an explicit state, not a hidden side effect.
        self._phase_lock_waveform_active = False
    ...
    def _seed_waveform_geometry(self, op_idx=0, row_idx=0, step_idx=0, salt="waveform"):
        """Seed-owned per-step waveform geometry.

        Unlike the older row-level field, this changes the *waveform parameters*
        on every active step while remaining deterministic for the literal seed.
        It is deliberately independent of the step gate so Phase-Lock can shape
        timbre/phase/frequency curvature without merely turning notes on/off.
        """
        import hashlib
        seed_text = self._seed_text() if hasattr(self, "_seed_text") else str(
            self.get_numeric_seed() if hasattr(self, "get_numeric_seed") else 0
        )
        payload = f"GROOVEBOX-WAVE-V1|{seed_text}|{salt}|{int(op_idx)}|{int(row_idx)}|{int(step_idx)}"
        digest = hashlib.blake2b(payload.encode("utf-8", "replace"), digest_size=32).digest()
        u = tuple(int.from_bytes(digest[i:i+8], "big") / float(2**64) for i in (0,8,16,24))
        phase = 2.0 * math.pi * (
            u[0] * PHI + u[1] * SQRT2 + u[2] * MEUM_LOG2 + u[3] * PHI_INV
        )
        # Bounded, musically useful controls. Each is seed+operator+row+step unique.
        return {
            "phase": float(phase),
            "harmonic_tilt": float((u[0] - 0.5) * 0.70),
            "stretch": float(1.0 + (u[1] - 0.5) * 0.095),
            "fm_depth": float(0.025 + 0.22 * u[2]),
            "am_depth": float(0.015 + 0.10 * u[3]),
            "curvature": float((u[2] - 0.5) * 0.52),
            "partial_phase": float(2.0 * math.pi * u[3]),
        }
    ...
    def _pkp_fire_step_hit(self, inst_name, step_idx, amp=1.0):
        ...
        try:
            ...
            # PKP test hit is a direct engine oscillator for the requested duration.
            # No hidden decay/click envelope is inserted.
            body = np.sin(2 * np.pi * freq * t)
            hit = body * float(amp)
            # PKP loudness is not fitted to another operator and is not peak-normalized.
            # Its programmed pad amplitude and master volume remain authoritative.
    ...
    def _on_live_source_changed(self, *args):
        """One recomposition per genuine user source change; no autonomous evolution."""
        if getattr(self, '_live_engine_update_guard', False):
            return
    ...
    def _ideal_phaselock_pattern(self, name, count, i, seed):
        ...
        for s in range(count):
            ...
            if on:
                ph = float(wf["phase_field"][s]) if wf and s < len(wf.get("phase_field", [])) else phase0
                hs = float(wf["harmonic_stretch"][s]) if wf and s < len(wf.get("harmonic_stretch", [])) else 1.0
                hb = float(wf["harmonic_balance"][s]) if wf and s < len(wf.get("harmonic_balance", [])) else 0.5
                sh = float(wf["shape_field"][s]) if wf and s < len(wf.get("shape_field", [])) else 0.5
                seed_h = 0.5 + 0.5 * math.sin(ph + (s + 1) * MEUM_LOG2)
                # Amplitude is deliberately not an envelope: it is a single engine value.
                amps[s] = float(np.clip(amps[s] if s < len(amps) and amps[s] > 0 else 0.72, 0.05, 1.0))
                pitches[s] = float(np.clip(2.0 ** ((seed_h - 0.5) * 0.58 + (hb - 0.5) * 0.24 + (hs - 1.0) * 0.20), 0.60, 1.60))
        return steps, amps, pitches
    ...
    def _apply_phase_lock_waveform_parameters(self):
        """Write the geometric synth-parameter field produced by Phase-Lock.

        This is intentionally separate from note gates/amplitudes. Phase-Lock therefore
        changes oscillator phase, harmonic stretch/tilt and curvature on the waveform
        itself instead of merely making the same four-note oscillator appear/disappear.
        No amplitude envelope or normalizer is created here.
        """
        count = int(self.spin_seq_length.value()) if hasattr(self, 'spin_seq_length') else 16
        for op_idx, name in enumerate(getattr(self, 'instrument_names_48', []) or []):
            mem = self.instrument_sequencer_memory.get(name)
            if not mem:
                continue
            self._ensure_seq_mem_length(mem, count)
            user_mask = self._user_pattern_mask(mem, count, instrument_name=name) if hasattr(self, '_user_pattern_mask') else [False]*count
            ph = mem.setdefault('phase_cells', [0.0]*count)
            hs = mem.setdefault('harmonic_stretch', [1.0]*count)
            ht = mem.setdefault('harmonic_tilt', [0.0]*count)
            cv = mem.setdefault('wave_curvature', [0.0]*count)
            ps = mem.setdefault('wave_partial_phase', [0.0]*count)
            for s in range(count):
                if s < len(user_mask) and user_mask[s]:
                    continue
                wg = self._seed_waveform_geometry(op_idx, 0, s, 'phase-lock-waveform')
                # Rotate the field again by the current literal seed + operator + step.
                # This makes the lock itself a geometric transform, not a copied waveform.
                u = self._seed_geometry(f'phase-lock-transform|{name}|{s}')
                ph[s] = float(wg['phase'] + 2.0*np.pi*(u[0]*PHI_INV + u[1]*MEUM_NORM))
                hs[s] = float(np.clip(wg['stretch'] * (0.985 + 0.030*u[2]), 0.91, 1.09))
                ht[s] = float(np.clip(wg['harmonic_tilt'] + (u[3]-0.5)*0.28, -0.50, 0.50))
                cv[s] = float(np.clip(wg['curvature'] + (u[0]-0.5)*0.20, -0.65, 0.65))
                ps[s] = float(wg['partial_phase'] + 2.0*np.pi*u[2])
        self._phase_lock_waveform_active = True
    ...
    def apply_unified_phase_lock(self, live=False):
        ...
        try:
            ...
            try:
                ...
                pass
            try:
                self._apply_phase_lock_waveform_parameters()
            except Exception as we:
                print(f"[Phase-Lock] waveform geometry: {we}")
            try:
                ...
                if user_hits:
                    ...
                    if wf_engine is not None:
                        ...
                        if hints and hints["euclidean"]:
                            base_p = 0.28 * (0.5 + 0.5 * math.sin(hints.get("phase", 0.0)))
    ...
    def _spectral_fit_voice(self, voice, target, amount=1.0):
        ...
        # Spectral fitting changes spectral/phase content only; it does not normalize
        # the fitted voice back to an arbitrary peak.
        if n < voice.size:
    ...
    def _render_mixdown_buffer(self, max_rows=None, progress="play"):
        ...
        # Keep the newest build's seed-only commissioning, but do it once.
        try:
            ...
            print(f"[Mixdown] auto-commission: {exc}")
        ...
        for row_idx in range(rows):
            ...
            for op_idx in active_cluster:
                ...
                # Per-sample waveform controls: Phase-Lock now acts on timbre as well
                # as gate timing. This prevents the same 4-note timbral cell from
                # repeating merely because the pattern length is a multiple of four.
                wave_stretch = np.ones_like(local_t, dtype=np.float64)
                wave_tilt = np.zeros_like(local_t, dtype=np.float64)
                wave_curvature = np.zeros_like(local_t, dtype=np.float64)
                wave_phase_mod = np.zeros_like(local_t, dtype=np.float64)
                ...
                for s_idx in range(min(int(seq_len), len(steps) if steps else int(seq_len))):
                    ...
                    # No synthetic ADSR/body envelope. The engine's programmed amplitude
                    # and probability are the complete loudness decision for this step.
                    step_env[s_mask] = amp * prob
                    pitch_track[s_mask] = pr

                    if getattr(self, '_phase_lock_waveform_active', False):
                        wg = {
                            "stretch": float(mem.get("harmonic_stretch", [1.0]*len(steps))[s_idx]) if s_idx < len(mem.get("harmonic_stretch", [])) else 1.0,
                            "harmonic_tilt": float(mem.get("harmonic_tilt", [0.0]*len(steps))[s_idx]) if s_idx < len(mem.get("harmonic_tilt", [])) else 0.0,
                            "curvature": float(mem.get("wave_curvature", [0.0]*len(steps))[s_idx]) if s_idx < len(mem.get("wave_curvature", [])) else 0.0,
                            "phase": float(mem.get("phase_cells", [0.0]*len(steps))[s_idx]) if s_idx < len(mem.get("phase_cells", [])) else 0.0,
                        }
                    else:
                        wg = self._seed_waveform_geometry(op_idx, row_idx, s_idx, "phase-lock-waveform")
                    wave_stretch[s_mask] = wg["stretch"]
                    wave_tilt[s_mask] = wg["harmonic_tilt"]
                    wave_curvature[s_mask] = wg["curvature"]
                    wave_phase_mod[s_mask] = wg["phase"]
                ...
                # Phase-Lock operates at the waveform level: every active step gets
                # its own deterministic harmonic tilt, phase cell, curvature and
                # slight partial stretch. The controls are bounded and energy-normalized.
                phase_mod = 0.045 * np.sin(wave_phase_mod + local_t * (1.0 + MEUM_LOG2 * 2.0))
                curved_phase = phase + phase_mod + wave_curvature * np.sin(phase * PHI_INV)
                partial_weights = np.array([0.90, 0.32, 0.20, 0.08, 0.055], dtype=np.float64)
                # Per-step spectral tilt changes the harmonic distribution without
                # changing total partial energy.
                partial_weights /= max(float(np.sqrt(np.sum(partial_weights ** 2))), 1e-9)
                partial_harmonics = (1.0, 2.0, 3.5, 4.0, 6.0)
                partials = np.zeros_like(curved_phase)
                for h_pos, (harmonic, wt) in enumerate(zip(partial_harmonics, partial_weights)):
                    # Tilt is applied as a bounded exponent to each partial.
                    tilt = np.clip(wave_tilt, -0.17, 0.17)
                    wt_field = wt * np.exp(-tilt * h_pos)
                    ph = curved_phase * harmonic * wave_stretch + phase0 * h_pos * MEUM_NORM + wave_phase_mod * (h_pos + 1) * 0.08
                    partials += wt_field * np.sin(ph)
                # Do not normalize the oscillator. Its RMS is an output of the actual
                # geometric harmonic parameters and remains part of the measured sound.
                osc = partials
                ...
                # No synthetic cluster normalization. Preserve the engine-calculated
                # amplitude relationships between simultaneous voices.
                row_mix += voice
        ...
        # No automatic master normalization or soft ceiling. Master volume and the
        # engine's own amplitudes are authoritative; preserve their actual RMS.
    ...
    def _meum_wave_from_playlist(self, n_samples=512, sample_rate=44100):
        ...
        # Preview waveform retains the engine's actual calculated amplitude; no
        # synthetic peak normalization is applied.
    ...
    def _export_wav_worker(self, file_path):
        ...
        try:
            ...
            # No automatic export normalization. Only the unavoidable PCM range clip
            # is applied at the file-format boundary.
